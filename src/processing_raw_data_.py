    ...
    for file_idx in range(len(input_file_2)):
        ...
        for col in sensor_cols:
            for func in agg_funcs:
                ...
                df1[new_col] = df1.groupby(windowdf)[col].transform(func).round(3)
        #-------------------------------------------------------------------- 

        # Signal magnitude area SMA  
        df1['window_id'] = (df1['time (ms)'] / window).astype(int) 
        ...
        #--------------------------------------------------------------------                       
        def compute_feat_per_window(df, time_col, window_size, columns_to_process,feats,choice):
            df = df.copy()
            df['window_id'] = (df[time_col] / window_size).astype(int)
            ...
            for window_id, group in grouped:
                for col in columns_to_process:
                    if choice == 0:
                        ...
                        val = np.real(abs(fft(group[col])))
                    elif choice == 6:
                        val = np.sum(abs(fft(group[col])**2))
                    df.loc[df['window_id'] == window_id, f'{feats[choice]}_{col}'] = val
            df[[f'{feats[choice]}_{col}' for col in columns_to_process]] = df[[f'{feats[choice]}_{col}' for col in columns_to_process]].round(3)
        ...
        choice = 0
        df1 = compute_feat_per_window(df1, 'time (ms)', window, sensor_cols,feats,choice)
