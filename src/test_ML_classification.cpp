...
#if USE_RAW_DATA
#include "RandomForest_RELIFF_FEATURES_10_Best.h"
...
//#define WINDOW 16 // Must be a power of 2
    ...
    for (;;) {
        ...
        if (data_flag == 0) {
            ...
            if (sample_index == WINDOW - 1) {
                ...
                float output_matrix[WINDOW][10]; // Initialize output matrix

                float f0 = 0, f1 = 0, f2 = 0, f3 = 0, f4[WINDOW], f5 = 0, f6 = 0, f7[WINDOW], f8 = 0,
                      f9 = 0; // initialize features

                f0 = window_abs_mean(acc_x_data, WINDOW) + window_abs_mean(acc_y_data, WINDOW)
                     + window_abs_mean(acc_z_data, WINDOW);     // Signal Magnitude Area Accelerometer
                f1 = compute_fft_energy(fft_mag_acc_y, WINDOW); // Energy_acceleration_y
                f2 = compute_iqr(gyro_z_data, WINDOW);          // IQR_gyro z
                f3 = compute_rms(acc_y_data, WINDOW);           // RMS_acceleration_y
                for (int i = 0; i < WINDOW; i++) {
                    f4[i] = production_cubic(acc_x_data[i], acc_y_data[i],
                                             acc_z_data[i]); // Acceleration Cubic Product Magnitude
                }
                f5 = compute_rms(acc_z_data, WINDOW); // RMS_acceleration_z
                f6 = compute_mad(acc_y_data, WINDOW); // MAD_acceleration y
                //f7 = fft_real_acc_y;                            // FFT_acceleration y
                f8 = compute_fft_energy(fft_mag_acc_z, WINDOW); // Energy_acceleration_z
                f9 = window_mean(acc_y_data, WINDOW);           // acceleration_y_window_mean
