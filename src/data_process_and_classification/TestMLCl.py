...
candidate_windows = [2,4,6,8,10,20,40,60,80,100,120,140]  # in sec (8s - 2min, 10s)
# ----------------------------- Sample Rate Dataset ---------------------------
# Available Datasets
# 1) 9.71 Hz
# 2) 10 Hz
...
train_size = 0.9  # or 1.0 if you want full-dataset mode
test = 1 -  train_size 
test_size = test
# ----------------------------- File Configuration ----------------------------
...
def stage_2(mode):
    ...
    fft_std_path = os.path.join(output_path_2, 'fft_std.csv')
    best_index_path = os.path.join('./', 'best_fft_index.json')
...
def stage_3(mode):
    ...
    def TrainTestSplit(X, y, train_size=0.75, test_size=0.25):
        ...
        '''
        if train_size == 1 and test_size == 0:
            print("[Stage 3] Full-dataset mode enabled: using 100% of data as training.")
    ...
    for file_idx, f in enumerate(feat_files):
        ...
        if series_of_experiments == 2:
            ...
            if 'good' in fname_lower:
                ...
                label_val = file_idx  # fallback
        else:
            label_val = file_idx  # for series_of_experiments == 1
        all_X.append(df_windowed)
        all_y.append(np.full(len(df_windowed), label_val))  # dataset number as label
...
def stage_5(cl, file_index):
    ...
    def capture_output_and_plot(classifier_name, accuracy, Data_tag, 
        ...
        if series_of_experiments == 1:
            ...
            '''
            original_class_names = [f'good_1_{sampleRate}', f'good_2_{sampleRate}', f'good_3_{sampleRate}',
                                    f'mid_1_{sampleRate}', f'mid_2_{sampleRate}.', f'mid_3_{sampleRate}',
                                    f'bad_1_{sampleRate}', f'bad_2_{sampleRate}', f'bad_3_{sampleRate}']
            '''
    ...
    X_train, X_test1, y_train, y_test1 = train_test_split(X_train, y_train, train_size = train_size , test_size = test_size )
...
'''
    # --- GA parameter space (max_features added) ---
    ga_param_space = {
        'DT': {
            'max_depth': list(range(1, 201)),
            'min_samples_split': list(range(2, 16)),
            'max_features': list(range(1, 61))
        },
        'RF': {
    ...
    def decode_genome(genome, clf_name):
        """Convert genome (list of floats) to hyperparameter dict."""
        space = ga_param_space[clf_name]
        params = {}
        keys = list(space.keys())
        for i, k in enumerate(keys):
            choices = space[k]
            idx = int(round(genome[i]))
            idx = max(0, min(len(choices) - 1, idx))
            params[k] = choices[idx]
        return params
    
    
    def make_fitness_fn(clf_name, X_train, y_train, X_val, y_val):
        base_clf = classifier
    
        def fitness_fn(ga_instance, solution, solution_idx):
            params = decode_genome(solution, clf_name)
            est = clone(base_clf)
            try:
    ...
    fitness_function = make_fitness_fn(classifier_name, X_train, y_train, X_val, y_val)
    keys = list(ga_param_space[classifier_name].keys())
    genome_length = len(keys)
    init_pop = np.array([[np.random.randint(0, len(ga_param_space[classifier_name][k])) for k in keys]
                         for _ in range(pop_size)], dtype=float)
    ...
    ga_instance = pygad.GA(
        ...
        mutation_percent_geness=mutation_percent_genes,
    ...
    print("Running Genetic Algorithm for hyperparameter search...")
    ga_instance.run()
    
    solution, solution_fitness, solution_idx = ga_instance.best_solution()
    best_params = decode_genome(solution, classifier_name)
    print("GA best params:", best_params)
    print("GA validation acc (fitness):", round(float(solution_fitness), 4))
    
    # --- Retrain final model on full X_train, evaluate on X_test ---
