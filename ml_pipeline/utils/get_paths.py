...
def get_paths(stage, sampleRate, classifier_name,series_of_experiments, mode="exp"):
    ...
    exp_path = f"../data/0_RAW/series_of_experiments_{series_of_experiments}/{sampleRate}_Hz_sampling/"
    ...
    if stage == 0:
        ...
        in_path = exp_path
        out_path = f"../data/1_CLEAN/{base}/{folder}/"
        out_files = [f"{fname.replace(".csv", f"{classifier_name}_clean.csv")}" for fname in files_to_use]
        plot_path = f"../output/pc_classification/plots/clean_data/{base}/{folder}/"
    elif stage == 1:
        in_files = [f"{fname.replace(".csv", f"{classifier_name}_clean.csv")}" for fname in files_to_use]
        in_path = f"../data/1_CLEAN/{base}/{folder}/"
        out_path = f"../data/2_FEATS_PREPROCESSSED/{base}/{folder}/"
        ...
        in_path = f"../data/2_FEATS_PREPROCESSSED/{base}/{folder}/"
        out_path = f"../data/3_FEATS/{base}/{folder}/"
        ...
        in_path = f"../data/3_FEATS/{base}/{folder}/"
        out_path = f"../data/4_FEATS_COMBINED/{base}/"
        out_files = []    
        plot_path = ''
    elif stage == 4:
        in_files = [f"X_data_{sampleRate}{classifier_name}.csv",
                 f"y_data_{sampleRate}{classifier_name}.csv"]
        in_path = f"../data/4_FEATS_COMBINED/{base}/"
        out_path = f"../data/5_FEATS_SELECTION/{base}/"
        time_path = exp_path 
        plot_path = f"../output/pc_classification/plots/feature_extraction/{base}/"
    else:
    ...
    return in_files, in_path, (out_files if stage != 4 else time_path), out_path, plot_path