...
def stage_4(window,window_size,sampleRate,classifier_name,series_of_experiments,plot_mode,matlab):
    ...
    input_file_4, input_path_4, time_path, output_path_4, plot_path = get_paths(4, sampleRate, classifier_name, series_of_experiments)
    ...
    if plot_mode == 1:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(plot_path, f'{series_of_experiments}{names[base_index]}_relieff_weights_plot_{sampleRate}{classifier_name}.png'), dpi=600)
        plt.show()
        plt.close()
    ...
    if os.path.isfile(times_file):
        ...
        if plot_mode == 1:
            ...
            plt.tight_layout()
            plt.savefig( os.path.join(plot_path, f'{series_of_experiments}{names[base_index]}_Feats_CustomScore_{sampleRate}{classifier_name}.png'), dpi=600)
            plt.show()
            plt.close()
